...
def stitching(input_file, folder):

    work_dir = './static/uploads/'+ folder + '/'
    stitch = ImageStitcher()

    for infile in input_file:
        stitch.add_image(work_dir + infile)
    result = stitch.stitch()
    #print(result.shape)
    resultfilename = work_dir +'result/' +'result.png'
    cv2.imwrite(resultfilename, cv2.cvtColor(result, cv2.COLOR_RGBA2BGRA))
